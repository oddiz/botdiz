...
export class GuildManager {
    ...
    async addGuild(guild: Guild): Promise<GuildController> {
        ...
        const controller = new GuildController(guild, this.client, this.repositories, this.shoukaku);
        ...
        try {
            ...
            this.logger.error('Failed to create guild controller', error as Error, { guildId: guild.id });
            throw error;
        }
    }
    ...
    async removeGuild(guildId: string): Promise<void> {
        ...
        try {
            ...
            this.logger.error('Failed to properly destroy guild controller', error as Error, { guildId });
        ...
        const shutdownPromises = Array.from(this.controllers.entries()).map(
            async ([guildId, controller]) => {
                try {
                    ...
                    this.logger.error('Error during controller shutdown', error as Error, { guildId });
                }
            }
        );
