...
import { DbUser, DbDiscordUser, AllowedGuild } from "../../../../server_src/db/databaseTypes";
...
export class UserRepository extends BaseRepository<DbUser> {
    ...
    async createUser(username: string, hashedPassword: string, avatarURL: string = '', isAdmin: boolean = false): Promise<DbUser> {
...
export class DiscordUserRepository extends BaseRepository<DbDiscordUser> {
    ...
    async findByUsernameAndDiscriminator(username: string, discriminator: string): Promise<DbDiscordUser | null> {
        return this.findOne({ 
            username, 
            discriminator 
    ...
    async updateDiscordProfile(discordId: string, profileData: {
        username?: string;
