import { Collection, Db, WithId, Document, Filter, UpdateFilter, InsertOneResult, UpdateResult, DeleteResult } from "mongodb";
...
export abstract class BaseRepository<T extends Document> implements IRepository<T> {
    ...
    async findById(id: string): Promise<T | null> {
        try {
            ...
            this.logger.error('Failed to find document by ID', error as Error, { id, collection: this.collectionName });
    ...
    async findOne(filter: Filter<T>): Promise<T | null> {
        try {
            ...
            this.logger.error('Failed to find document', error as Error, { filter, collection: this.collectionName });
    ...
    async findMany(filter: Filter<T>, limit?: number, skip?: number): Promise<T[]> {
        try {
            ...
            this.logger.error('Failed to find documents', error as Error, { 
                filter, limit, skip, collection: this.collectionName 
            });
    ...
    async create(entity: Omit<T, '_id'>): Promise<T> {
        try {
            ...
            this.logger.debug('Created document', { id: result.insertedId, collection: this.collectionName });
            return created;
        } catch (error) {
    ...
    async update(id: string, updates: Partial<T>): Promise<T> {
        try {
            ...
            this.logger.error('Failed to update document', error as Error, { 
                id, updates: this.sanitizeForLog(updates), collection: this.collectionName 
            });
    ...
    async updateOne(filter: Filter<T>, updates: UpdateFilter<T>): Promise<UpdateResult> {
        try {
            ...
            this.logger.error('Failed to update documents', error as Error, { 
                filter, updates, collection: this.collectionName 
            });
    ...
    async delete(id: string): Promise<boolean> {
        try {
            ...
            this.logger.error('Failed to delete document', error as Error, { id, collection: this.collectionName });
    ...
    async deleteOne(filter: Filter<T>): Promise<boolean> {
        try {
            ...
            this.logger.error('Failed to delete document', error as Error, { filter, collection: this.collectionName });
    ...
    async exists(filter: Filter<T>): Promise<boolean> {
        try {
            ...
            this.logger.error('Failed to check document existence', error as Error, { 
                filter, collection: this.collectionName 
            });
