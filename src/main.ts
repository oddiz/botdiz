    ...
    client.on('ready', async () => {
        if (client.user) {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                if (client.user.username !== 'botdiz [alpha]') {
        ...
        for (const guild of client.guilds.cache) {
            ...
            logger.log(
                'info',
                `Creating controller for guild: ${guild[1].name}.`
            )
        }
    ...
    client.on('messageCreate', (message) => {
        ...
        if (message.guild) {
            ...
            const guildController = GuildControllers.find(
                ({ guildId }) => guildId === messageGuildId
            )?.controller
            ...
            if (guildController) {
                ...
                logger.log(
                    'warn',
                    `No controller found for guild: ${message.guild.name}.`
                )
            }
        }
    })
    ...
    client.on('interactionCreate', (interaction) => {
        if (interaction.guild) {
            ...
            if (guildController) {
                ...
                logger.log(
                    'warn',
                    `No controller found for guild: ${interaction.guild.name}.`
                )
            }
        }
    })
