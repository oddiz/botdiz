...
export interface MusicEvents {
    ...
    'vote.added': (data: { voteId: string; userId: string; currentVotes: number; requiredVotes: number }) => void;
...
export class MusicEventBus extends TypedEmitter<MusicEvents> {
    ...
    emitEvent<K extends keyof MusicEvents>(
        event: K, 
        data: Parameters<MusicEvents[K]>[0]
    ): boolean {
