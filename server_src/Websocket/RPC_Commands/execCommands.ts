...
export default {
    ...
    RPC_resumePlayer: async function (user: DbDiscordUser, guildId: string): Promise<ExecCommandResponse> {
        try {
            ...
            if (!guildMusicController) throw 'Guild music controller not found for id: ' + guildId;
    ...
    RPC_skipSong: async function (
        ...
        try {
            if (user.discord_id) {
                ...
                const result =
                    await guildMusicController.SkipHandler.handleInterface(
                        user.discord_id,
                        skipAmount
                    );
    ...
    RPC_addSpotifyPlaylist: async function (
        ...
        try {
            ...
            if (!guildController?.MusicController) throw 'Guild music controller not found for id: ' + guildId; 
    ...
    RPC_shuffleQueue: async function (guildId: string): Promise<ExecCommandResponse> {
        try {
            const guildMusicController = await GuildControllers.find(
                (element) => element.guildId === guildId
            )?.controller.MusicController;
